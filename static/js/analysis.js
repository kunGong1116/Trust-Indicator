...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    // 先获取图片信息和详情
    let imageDetailPromise = fetch(`/getimagedetail/${imageId}`, {
        credentials: "include",
    })
        .then((response) => {
    ...
    document.querySelector(
        "#circle-Original"
    ).style.strokeDasharray = `${scoreNormalized_original}, 100`;
    document.querySelector(
        "#percentage-Original"
    ).textContent = `${score_original}%`;

    document.querySelector(
        "#circle-AIGC"
    ).style.strokeDasharray = `${scoreNormalized_aigc}, 100`;
    document.querySelector("#percentage-AIGC").textContent = `${score_aigc}%`;

    document.querySelector(
        "#circle-Manipulation"
    ).style.strokeDasharray = `${scoreNormalized_manipulation}, 100`;
    document.querySelector(
        "#percentage-Manipulation"
    ).textContent = `${score_manipulation}%`;
    ...
    if (score_manipulation > 30) {
        document.getElementById(
            "signal1"
        ).style.backgroundImage = `url('/static/images/aigc.png')`;
    ...
    if (score_original > 50) {
        ...
        document.getElementById(
            "signal2"
        ).style.backgroundImage = `url('/static/images/${secondIndicatorImages[0]}')`; // ORIGIN 图标
    } else if (score_aigc > 50) {
        // 如果 AIGC 分数大于 50，显示 AIGC 图标
        document.getElementById(
            "signal2"
        ).style.backgroundImage = `url('/static/images/${secondIndicatorImages[1]}')`; // AIGC 图标
    } else {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    imageDisplayDiv.style.backgroundSize = "cover";
    ...
    if (imageId) {
        ...
        fetch(`/getimagedetail/${imageId}`, { credentials: "include" })
            ...
            .then((data) => {
                const description =
                    data.ImageDescription || "No description provided.";
    ...
    if (imageBase64) {
        ...
        // 可选：用完后清除 sessionStorage（避免内存泄漏）
        sessionStorage.removeItem("uploadedImageBase64");
    } else {
        // 如果没有缓存，则走原来的逻辑（请求 /getImage）
        fetch("/getImage", { credentials: "include" })
            .then((response) => response.blob())
            .then((imageBlob) => {
                const imageObjectURL = URL.createObjectURL(imageBlob);
                imageDisplay.style.backgroundImage = `url(${imageObjectURL})`;
            });
    }
