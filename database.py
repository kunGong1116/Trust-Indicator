...
class User(UserMixin, db.Model):
    ...
    trust_profile = db.relationship(
        "TrustProfile", backref="user", uselist=False, lazy=True
    )
...
class TrustSnippet(db.Model):
    ...
    profile_id = db.Column(
        db.Integer, db.ForeignKey("trust_profile.id"), nullable=False
    )
    snippet_type = db.Column(
        db.String(50), nullable=False
    )  # 'ai_threshold', 'megadata_complete'
    settings = db.Column(
        db.Text
    )  # Used to store settings in JSON format, such as the specific value of AI threshold
