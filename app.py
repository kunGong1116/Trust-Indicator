...
# Create Trust Profile tables if they don't exist
with app.app_context():
    try:
        # Check if tables exist by querying them
        TrustProfile.query.first()
        app.logger.info("Trust Profile tables already exist")
    except Exception as e:
        app.logger.info("Creating Trust Profile tables")
        # Use the standard create_all method without specifying tables
        db.create_all()
        app.logger.info("Trust Profile tables created successfully")
...
@app.route('/api/detect_aigc', methods=['POST'])
@login_required
def detect_aigc():
    """
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': f'AIGC检测失败: {str(e)}'
        }), 500


@app.route('/get_trust_profile')
@login_required
def get_trust_profile():
    ...
    try:
        ...
        if not trust_profile:
            ...
            try:
                trust_profile = TrustProfile(user_id=user_id, last_updated=datetime.utcnow())
        ...
        # Log the error
        app.logger.error(f"Error getting trust profile: {str(e)}")
...
@app.route('/save_trust_profile', methods=['POST'])
@login_required
def save_trust_profile():
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': 'Trust Profile saved successfully'})
    except Exception as e:
        # Log the error for debugging
        app.logger.error(f"Error saving trust profile: {str(e)}")
        # Return user-friendly error
        return jsonify({'status': 'error', 'message': f'Error saving Trust Profile: {str(e)}'}), 500

@app.route('/get_available_snippets')
@login_required
def get_available_snippets():
    ...
    available_snippets = [
        ...
        },
        {
            ...
            'description': 'Reject photos with incomplete MetaData',
...
@app.route('/trust_profile_edit')
@login_required
def trust_profile_edit():
    """Trust Profile edit page"""
    return render_template('html/userprofile.html', user=current_user, show_trust_editor=True)
